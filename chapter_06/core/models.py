...
class LossFn:
    ...
    def focal_Loss(self,gt_label,pred_label):#focal_Los
        w = 1e-6
        pred_label = torch.squeeze(pred_label)
        gt_label = torch.squeeze(gt_label)
        # get the mask element which >= 0, only 0 and 1 can effect the detection loss
...
class PNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.apply(weights_init) # weight initiation with xavier
    def forward(self, x):
        x = (x - 127.5) / 128.
        x = self.pre_layer(x)
...
class ONet(nn.Module):
    ...
    def __init__(self,is_train=False, use_cuda=True):
        ...
        self.conv6_1 = nn.Linear(256, 1)# detection
        self.conv6_2 = nn.Linear(256, 4)# bounding box regression
        self.apply(weights_init) # weight initiation weih xavier
    def forward(self, x):
        x = (x - 127.5) / 128.
        x = self.pre_layer(x)
        x = x.view(x.size(0), -1)
